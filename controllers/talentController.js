  ...
  try {
    ...
    const response = {
      talent: {
        ...
        userName: user.username,
        minAvailableHours: user.availability || 0,
...
exports.getTalents = async (req, res) => {
  try {
    ...
    // Format response
const formattedTalents = await Promise.all(
  rows.map(async (row) => {
    ...
    let parsedSkills = [];
    if (row.talent_skills) {
      if (typeof row.talent_skills === 'string') {
        parsedSkills = JSON.parse(row.talent_skills);
      } else if (Array.isArray(row.talent_skills)) {
        parsedSkills = row.talent_skills;
      }
    }

    const serviceName = await resolveTalentSkills(parsedSkills);
